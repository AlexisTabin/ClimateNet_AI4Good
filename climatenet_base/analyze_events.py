...
def analyze_events(event_masks_xarray, class_masks_xarray, results_dir):
    ...
    def average_location(coordinates_pixel):
        ...
        """
        coordinates_degree = [pixel_to_degree(
            cord) for cord in coordinates_pixel]
        ...
        average_degree = math.degrees(
            central_latitude), math.degrees(central_longitude)
    ...
    pool_result = pool.starmap(
        event_type_of_mask, zip(event_masks, class_masks))
    ...
    # sum over all AR genesis events
    genesis_count_ar = np.zeros(event_masks.shape[1:3])
    ...
    for time in range(event_masks.shape[0]):
        genesis_events = np.isin(
            event_masks[time], genesis_ids_per_time.get(time, []))
        termination_events = np.isin(
            event_masks[time], termination_ids_per_time.get(time, []))
    ...
    for event_class in ['tc', 'ar']:
        ...
        genesis_times = np.array([genesis_time_per_id[event_id]
                                 for event_id in this_class_ids])
        ...
        plt.title(f"Lifetime histogram of {event_class.upper():s}s", fontdict={
                  'fontsize': 16})
    ...
    def visualize_frequency_map(frequency_map, title, colorbar_text, filepath):
        ...
        contourf = mymap.contourf(lon, lat,
                                  np.ma.masked_array(
                                      frequency_map, mask=(frequency_map == 0)),
                                  levels=np.linspace(
                                      0.0, frequency_map.max(), 11),
                                  alpha=0.7)
