...
def track_events(class_masks_xarray, minimum_time_length=5,
    ...
    def identify_components(time):
        ...
        for i in range(np.shape(class_mask)[0]):
            for j in range(np.shape(class_mask)[1]):
                ...
                # label connected component with new id with BFS
                if class_type != 0 and event_mask[i][j] == 0:
                    frontier = [(i, j)]
                    ...
                    while len(frontier) > 0:
                        ...
                        for neighbor_row in range(row-1, row+2):
                            ...
                            for neighbor_col in range(col-1, col+2):
                                ...
                                # don't propagate to other type
                                if class_mask[neighbor_row][neighbor_col] != class_type:
                                    continue
                                # not yet considered
                                if event_mask[neighbor_row][neighbor_col] == 0:
    ...
    pixels_per_event_id_per_time = [{} for x in range(
        len(event_masks))]  # one pixel per event Id
            ...
            for j in range(np.shape(event_mask)[1]):
                ...
                if id < 1000:
                    ...
                    if above_threshold:
                        ...
                        # set component class to background
                        label_component(class_mask, i, j, 0)
                ...
                for future_time in range(time+1, min(time+1+future_lookup_range, len(event_masks))):
                    ...
                    if class_mask[i][j] == future_class_mask[i][j] \
                        ...
                        above_threshold = label_component(
                            future_event_mask, i, j, id, drop_threshold(class_mask, i, j))
                        if above_threshold:
                            ...
                            if id in pixels_per_event_id_per_time[future_time].keys():
                                ...
                                pixels_per_event_id_per_time[future_time][id].append(
                                    (i, j))
                            else:
                                pixels_per_event_id_per_time[future_time][id] = [
                                    (i, j)]
